...
session_start();
if(empty($_POST['nome'])){
    $_SESSION['vazio_nome_De'] = "Campo nome é obrigatório";
    $url = 'http://localhost/fotografo/admin/Registar-Depoimento.php';
    echo "
        <META HTTP-EQUIV=REFRESH CONTENT = '0;URL=$url'>
    ";
}else{
    $_SESSION['value_nome_De'] = $_POST['nome'];
}


if(empty($_POST['descricao'])){
    $_SESSION['vazio_descricao_De'] = "campo descricao é obrigatório";
    $url = 'http://localhost/fotografo/admin/Registar-Depoimento.php';
    echo "
        <META HTTP-EQUIV=REFRESH CONTENT = '0;URL=$url'>
    ";
}else{
    $_SESSION['value_descricao_De'] = $_POST['descricao'];
}



if(empty($_FILES['arquivo']['name'])){
    $_SESSION['vazio_arquivo_De'] = "campo foto é obrigatório";
    $url = 'http://localhost/fotografo/admin/Registar-Depoimento.php';
    echo "
        <META HTTP-EQUIV=REFRESH CONTENT = '0;URL=$url'>
    ";
}else{
    $_SESSION['value_arquivo_De'] = $_FILES['arquivo']['name'];
}
...
echo "
        <META HTTP-EQUIV=REFRESH CONTENT = '0;URL=http://localhost/fotografo/admin/Registar-Depoimento.php'>
...
if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'] . $nome_final)) {
    ...
    if($cat->insert()){
        ...
        <script type=\"text/javascript\">
            alert(\"Depoimento registado com sucesso.\");
        </script>
        ";
        ...
        <META HTTP-EQUIV=REFRESH CONTENT = '0;URL=http://localhost/fotografo/admin/Registar-Depoimento.php'>
        <script type=\"text/javascript\">
            alert(\"Depoimento não registado.\");
        </script>
        ";
    }
} else {
